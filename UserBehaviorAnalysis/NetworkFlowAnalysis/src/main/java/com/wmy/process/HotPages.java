package com.wmy.process;

import com.wmy.beans.ApacheLogEvent;
import com.wmy.beans.PageViewCount;
...
/**
 * ClassName:HotPages
 * Package:com.wmy.beans.process
 *
 * @date:2021/6/24 21:59
 * @author:数仓开发工程师
 * @email:2647716549@qq.com
 * @Description: 热门页面 --- 只有1w条数据
 */
public class HotPages {
    public static void main(String[] args) throws Exception {
        ...
        DataStream<String> inputStream = env.socketTextStream("yaxin01", 8888);

        DataStream<ApacheLogEvent> dataStream = inputStream
                .map(line -> {
                    String[] fields = line.split(" ");
                    SimpleDateFormat simpleDateFormat = new SimpleDateFormat("dd/MM/yyyy:HH:mm:ss");
                    Long timestamp = simpleDateFormat.parse(fields[3]).getTime(); // 转换为毫秒值
                    return new ApacheLogEvent(fields[0], fields[1], timestamp, fields[5], fields[6]);
                })
                .assignTimestampsAndWatermarks(new BoundedOutOfOrdernessTimestampExtractor<ApacheLogEvent>(Time.seconds(1)) { // 延迟一分钟
                    @Override
                    public long extractTimestamp(ApacheLogEvent element) {
                        return element.getTimestamp(); // 看它是否是毫秒
                    }
                });
